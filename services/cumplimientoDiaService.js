...
const getCumplimientoDiaVendedor = async (codigoVendedor, filters = {}) => {
    ...
    // Query para obtener cuota y ventas del vendedor
    ...
    const query = `
        ...
        LEFT JOIN (
            SELECT SUM(COALESCE(cuota_dia, 0)) AS cuota_dia_total
            FROM "cuotaDia"
            WHERE id_usuario = :idUsuario AND fecha_fin = :fechaCuota
        ) cd ON true
        LEFT JOIN (
            SELECT
                SUM(
    ...
    replacements.idVendedor = vendedor.id_vendedor;
    ...
    const row = rows[0] || { cuota_dia_total: 0, venta_acum: 0 };
...
const getCumplimientoDiaVendedores = async (filters = {}) => {
    ...
    const query = `
        ...
        WHERE (cd.cuota_dia_total IS NOT NULL OR vt.venta_acum IS NOT NULL)
        ${vendedorFilter}
