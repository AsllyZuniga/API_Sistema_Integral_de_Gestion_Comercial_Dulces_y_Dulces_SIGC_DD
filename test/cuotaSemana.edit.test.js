...
async function createTestUsuario({ id_rol = 1, acceso_cuotas = true } = {}) {
    ...
    const username = `test_semana_${suffix}`;
...
async function createTestVendedor(usuario, overrides = {}) {
    ...
    const vendedor = await vendedor_model.create({
        codigo_vendedor: `VS${suffix}`,
        nombre: `Test Vendedor Semana ${suffix}`,
...
async function createCuotaSemana(usuario, overrides = {}) {
    const cuota = await cuotaSemana_model.create({
        cuota_semana: 250000,
...
describe('cuotaSemana edit - backend', { concurrency: false }, () => {
    before(async () => {
    ...
    describe('service.updateById', () => {
        it('modifica cuota_semana y conserva id, id_usuario y fechas', async () => {
            const usuario = await createTestUsuario();
            await createTestVendedor(usuario);
            const cuota = await createCuotaSemana(usuario, {
                cuota_semana: 250000,
                fecha_inicio: '2026-03-03',
                fecha_fin: '2026-03-08'
            });

            const updated = await cuotaSemanaService.updateById(cuota.id_cuotaSemana, {
                cuota_semana: 500000
            });

            assert.strictEqual(updated.id_cuotaSemana, cuota.id_cuotaSemana);
            assert.strictEqual(Number(updated.id_usuario), usuario.id_usuario);
            assert.strictEqual(Number(updated.cuota_semana), 500000);
            assert.strictEqual(String(updated.fecha_inicio), '2026-03-03');
            assert.strictEqual(String(updated.fecha_fin), '2026-03-08');

            const fromDb = await cuotaSemana_model.findByPk(cuota.id_cuotaSemana);
            assert.strictEqual(Number(fromDb.cuota_semana), 500000);
        });
        ...
        it('modifica fechas manteniendo vendedor y valor', async () => {
            ...
            assert.strictEqual(updated.id_cuotaSemana, cuota.id_cuotaSemana);
            assert.strictEqual(Number(updated.id_usuario), usuario.id_usuario);
            assert.strictEqual(Number(updated.cuota_semana), 250000);
            assert.strictEqual(String(updated.fecha_inicio), '2026-03-10');
            assert.strictEqual(String(updated.fecha_fin), '2026-03-15');
        });

        it('error 404 si la cuota no existe', async () => {
            await assert.rejects(
                async () => cuotaSemanaService.updateById(999999999, { cuota_semana: 1 }),
        ...
        it('error 400 si se intenta cambiar id_usuario', async () => {
            ...
            await assert.rejects(
                async () => cuotaSemanaService.updateById(cuota.id_cuotaSemana, { id_usuario: 99999 }),
                (err) => err.statusCode === 400 && err.message.includes('No se permite cambiar')
            );
        });

        it('error 400 con cuota_semana negativa', async () => {
            const usuario = await createTestUsuario();
            const cuota = await createCuotaSemana(usuario);

            await assert.rejects(
                async () => cuotaSemanaService.updateById(cuota.id_cuotaSemana, { cuota_semana: -100 }),
                (err) => err.statusCode === 400 && err.message.includes('entero mayor o igual a 0')
            );
        });

        it('error 400 con cuota_semana no numérica', async () => {
            const usuario = await createTestUsuario();
            const cuota = await createCuotaSemana(usuario);

            await assert.rejects(
                async () => cuotaSemanaService.updateById(cuota.id_cuotaSemana, { cuota_semana: 'abc' }),
                (err) => err.statusCode === 400 && err.message.includes('entero mayor o igual a 0')
            );
        });
        ...
        it('error 400 con fecha_inicio mayor que fecha_fin', async () => {
            ...
            await assert.rejects(
                async () => cuotaSemanaService.updateById(cuota.id_cuotaSemana, {
        ...
        it('error 409 si el nuevo período coincide con otra cuota del mismo usuario', async () => {
            const usuario = await createTestUsuario();
            const cuotaA = await createCuotaSemana(usuario, {
                fecha_inicio: '2026-03-03',
                fecha_fin: '2026-03-08'
            });
            const cuotaB = await createCuotaSemana(usuario, {
                fecha_inicio: '2026-03-10',
                fecha_fin: '2026-03-15'
            });

            await assert.rejects(
                async () => cuotaSemanaService.updateById(cuotaB.id_cuotaSemana, {
                    fecha_inicio: '2026-03-03',
                    fecha_fin: '2026-03-08'
                }),
        ...
        it('no afecta cuotas de otros vendedores', async () => {
            ...
            const cuotaA = await createCuotaSemana(usuarioA, { cuota_semana: 100000 });
            const cuotaB = await createCuotaSemana(usuarioB, { cuota_semana: 200000 });

            await cuotaSemanaService.updateById(cuotaA.id_cuotaSemana, { cuota_semana: 9999999 });

            const unchanged = await cuotaSemana_model.findByPk(cuotaB.id_cuotaSemana);
            assert.strictEqual(Number(unchanged.cuota_semana), 200000);
        });
        ...
        it('regresión: deleteById sigue funcionando después de editar', async () => {
            ...
            await cuotaSemanaService.updateById(cuota.id_cuotaSemana, { cuota_semana: 5555 });
            const deleted = await cuotaSemanaService.deleteById(cuota.id_cuotaSemana);
            assert.ok(deleted);
        ...
        it('regresión: create sigue funcionando', async () => {
            ...
            assert.strictEqual(Number(created.cuota_semana), 12345);
            assert.strictEqual(String(created.fecha_inicio), '2026-05-04');
    ...
    describe('controller.update', () => {
        it('responde success true con datos actualizados', async () => {
            const usuario = await createTestUsuario();
            const cuota = await createCuotaSemana(usuario);
            const req = { params: { id: cuota.id_cuotaSemana }, body: { cuota_semana: 7777 } };
            const res = createRes();

            await cuotaSemanaController.update(req, res);

            assert.strictEqual(res.statusCode, 200);
            assert.strictEqual(res.payload.success, true);
            assert.strictEqual(Number(res.payload.data.cuota_semana), 7777);
            assert.strictEqual(res.payload.message, 'Cuota semanal actualizada correctamente');
        });

        it('mapea not found a 404', async () => {
            const req = { params: { id: 999999999 }, body: { cuota_semana: 1 } };
            const res = createRes();
        ...
        it('mapea validación a 400', async () => {
            const usuario = await createTestUsuario();
            const cuota = await createCuotaSemana(usuario);
            const req = { params: { id: cuota.id_cuotaSemana }, body: { cuota_semana: -1 } };
            const res = createRes();
        ...
        it('mapea duplicado a 409', async () => {
            const usuario = await createTestUsuario();
            await createCuotaSemana(usuario, {
                fecha_inicio: '2026-03-03',
                fecha_fin: '2026-03-08'
            });
            const cuotaB = await createCuotaSemana(usuario, {
            ...
            const req = {
                params: { id: cuotaB.id_cuotaSemana },
                body: { fecha_inicio: '2026-03-03', fecha_fin: '2026-03-08' }
            };
            const res = createRes();
